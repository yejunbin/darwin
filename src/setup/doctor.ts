import { getUserName as getBioUserName, isLoggedIn as isBioLoggedIn } from "@companion-ai/alpha-hub/lib";
...
export type DarwinStatusSnapshot = {
	model?: string;
	modelValid: boolean;
...
export function collectStatusSnapshot(options: DoctorOptions): DarwinStatusSnapshot {
	...
	return {
		...
		alphaLoggedIn: isBioLoggedIn(),
		alphaUser: isBioLoggedIn() ? getBioUserName() ?? undefined : undefined,
...
export function runStatus(options: DoctorOptions): void {
	...
	printInfo(`bioRxiv: ${snapshot.alphaLoggedIn ? snapshot.alphaUser ?? "configured" : "not configured"}`);
...
export function runDoctor(options: DoctorOptions): void {
	...
	console.log("");
	console.log(`bioRxiv auth: ${isBioLoggedIn() ? "ok" : "missing"}`);
	if (isBioLoggedIn()) {
	...
	console.log("setup hint: darwin setup");
}
