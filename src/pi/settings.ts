...
export function readJson(path: string): Record<string, unknown> {
	...
	try {
		...
		if (process.env.DARWIN_DEBUG === "1") {
			process.stderr.write(
				`[darwin] warning: failed to parse ${path}, treating as empty (${error instanceof Error ? error.message : "unknown error"})\n`,
			);
		}
		return {};
	}
...
export function normalizeDarwinSettings(
	settingsPath: string,
