...
function Resolve-VersionMetadata {
  ...
  return [PSCustomObject]@{
    ...
    DownloadUrl = if ($env:DARWIN_INSTALL_SKILLS_ARCHIVE_URL) { $env:DARWIN_INSTALL_SKILLS_ARCHIVE_URL } else { "https://github.com/getcompanion-ai/darwin/archive/refs/tags/v$resolvedVersion.zip" }
  }
}
...
function Resolve-InstallDir {
  ...
  if ($ResolvedScope -eq "Repo") {
    return Join-Path (Get-Location) ".agents\skills\darwin"
  }
  if ($ResolvedScope -eq "OpenCode") {
    return Join-Path (Get-Location) ".opencode\skills\darwin"
  }

  $codexHome = if ($env:CODEX_HOME) { $env:CODEX_HOME } else { Join-Path $HOME ".codex" }
  return Join-Path $codexHome "skills\darwin"
}
...
$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("darwin-skills-install-" + [System.Guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $tmpDir | Out-Null

try {
  $archivePath = Join-Path $tmpDir "darwin-skills.zip"
  $extractDir = Join-Path $tmpDir "extract"

  Write-Host "==> Downloading Darwin skills $resolvedVersion"
  ...
  if (-not $sourceRoot) {
    throw "Could not find extracted Darwin archive."
  }
  ...
  Write-Host "Darwin skills $resolvedVersion installed successfully."
} finally {
  if (Test-Path $tmpDir) {
