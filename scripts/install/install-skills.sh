...
VERSION="latest"
SCOPE="${DARWIN_SKILLS_SCOPE:-user}"
TARGET_DIR="${DARWIN_SKILLS_DIR:-}"
...
download_file() {
  ...
  echo "curl or wget is required to install Darwin skills." >&2
  exit 1
}
...
resolve_target_dir() {
  ...
  case "$SCOPE" in
    repo)
      printf '%s/.agents/skills/darwin\n' "$PWD"
      ;;
    opencode)
      printf '%s/.opencode/skills/darwin\n' "$PWD"
      ;;
    user)
      codex_home="${CODEX_HOME:-$HOME/.codex}"
      printf '%s/skills/darwin\n' "$codex_home"
      ;;
    *)
...
archive_url="${DARWIN_INSTALL_SKILLS_ARCHIVE_URL:-}"
if [ -z "$archive_url" ]; then
  case "$git_ref" in
    main)
      archive_url="https://github.com/getcompanion-ai/darwin/archive/refs/heads/main.tar.gz"
      ;;
    v*)
      archive_url="https://github.com/getcompanion-ai/darwin/archive/refs/tags/${git_ref}.tar.gz"
      ;;
  esac
fi
...
step "Installing Darwin skills ${resolved_version} (${SCOPE})"
...
archive_path="$tmp_dir/darwin-skills.tar.gz"
...
printf 'Darwin skills %s installed successfully.\n' "$resolved_version"
