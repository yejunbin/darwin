...
VERSION="${1:-latest}"
INSTALL_BIN_DIR="${DARWIN_INSTALL_BIN_DIR:-$HOME/.local/bin}"
INSTALL_APP_DIR="${DARWIN_INSTALL_APP_DIR:-$HOME/.local/share/darwin}"
SKIP_PATH_UPDATE="${DARWIN_INSTALL_SKIP_PATH_UPDATE:-0}"
path_action="already"
path_profile=""
...
download_file() {
  ...
  echo "curl or wget is required to install Darwin." >&2
  exit 1
}
...
add_to_path() {
  ...
  profile="${DARWIN_INSTALL_SHELL_PROFILE:-$HOME/.profile}"
  if [ -z "${DARWIN_INSTALL_SHELL_PROFILE:-}" ]; then
    case "${SHELL:-}" in
      */zsh)
        profile="$HOME/.zshrc"
        ;;
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install Darwin." >&2
    exit 1
  fi
}

warn_command_conflict() {
  expected_path="$INSTALL_BIN_DIR/darwin"
  resolved_path="$(command -v darwin 2>/dev/null || true)"
  ...
  if [ "$resolved_path" != "$expected_path" ]; then
    step "Warning: current shell resolves darwin to $resolved_path"
    step "Run now: export PATH=\"$INSTALL_BIN_DIR:\$PATH\" && hash -r && darwin"
...
resolve_release_metadata() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    release_page="$(download_text "https://github.com/getcompanion-ai/darwin/releases/latest")"
    resolved_version="$(printf '%s\n' "$release_page" | sed -n 's@.*releases/tag/v\([0-9][^"<>[:space:]]*\).*@\1@p' | head -n 1)"

    if [ -z "$resolved_version" ]; then
      echo "Failed to resolve the latest Darwin release version." >&2
      exit 1
    fi
  else
  ...
  bundle_name="darwin-${resolved_version}-${asset_target}"
  archive_name="${bundle_name}.${archive_extension}"
  download_url="${DARWIN_INSTALL_BASE_URL:-https://github.com/getcompanion-ai/darwin/releases/download/v${resolved_version}}/${archive_name}"
...
step "Installing Darwin ${resolved_version} for ${asset_target}"
...
Workarounds:
  ...
  - pass the latest published version explicitly, e.g.:
    curl -fsSL https://darwin.is/install | bash -s -- 0.2.31
EOF
  exit 1
fi
...
step "Linking darwin into $INSTALL_BIN_DIR"
cat >"$INSTALL_BIN_DIR/darwin" <<EOF
#!/bin/sh
set -eu
exec "$INSTALL_APP_DIR/$bundle_name/darwin" "\$@"
EOF
chmod 0755 "$INSTALL_BIN_DIR/darwin"
...
case "$path_action" in
  ...
  *)
    ...
    step "Run: hash -r && darwin"
    ;;
esac

warn_command_conflict

printf 'Darwin %s installed successfully.\n' "$resolved_version"
