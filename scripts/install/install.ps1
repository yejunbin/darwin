...
function Resolve-LatestReleaseVersion {
  $page = Invoke-WebRequest -Uri "https://github.com/getcompanion-ai/darwin/releases/latest"
  $match = [regex]::Match($page.Content, 'releases/tag/v([0-9][^"''<>\s]*)')
  if (-not $match.Success) {
    throw "Failed to resolve the latest Darwin release version."
  }
...
function Resolve-ReleaseMetadata {
  ...
  $bundleName = "darwin-$resolvedVersion-$AssetTarget"
  $archiveName = "$bundleName.$BundleExtension"
  $baseUrl = if ($env:DARWIN_INSTALL_BASE_URL) { $env:DARWIN_INSTALL_BASE_URL } else { "https://github.com/getcompanion-ai/darwin/releases/download/v$resolvedVersion" }
...
$installRoot = Join-Path $env:LOCALAPPDATA "Programs\darwin"
$installBinDir = Join-Path $installRoot "bin"
$bundleDir = Join-Path $installRoot $bundleName

$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("darwin-install-" + [System.Guid]::NewGuid().ToString("N"))
...
Workarounds:
  ...
  - pass the latest published version explicitly, e.g.:
    & ([scriptblock]::Create((irm https://darwin.is/install.ps1))) -Version 0.2.31
"@
  }
  ...
  $shimPath = Join-Path $installBinDir "darwin.cmd"
  $shimPs1Path = Join-Path $installBinDir "darwin.ps1"
  Write-Host "==> Linking darwin into $installBinDir"
  @"
@echo off
CALL "$bundleDir\darwin.cmd" %*
"@ | Set-Content -Path $shimPath -Encoding ASCII

  @"
`$BundleDir = "$bundleDir"
& "`$BundleDir\node\node.exe" "`$BundleDir\app\bin\darwin.js" @args
"@ | Set-Content -Path $shimPs1Path -Encoding UTF8
  ...
  if (-not $alreadyOnPath) {
    ...
    Write-Host "Updated user PATH. Open a new shell to run darwin."
  } else {
    Write-Host "$installBinDir is already on PATH."
  }

  $resolvedCommand = Get-Command darwin -ErrorAction SilentlyContinue
  if ($resolvedCommand -and $resolvedCommand.Source -ne $shimPath) {
    Write-Warning "Current shell resolves darwin to $($resolvedCommand.Source)"
    Write-Host "Run in a new shell, or run: `$env:Path = '$installBinDir;' + `$env:Path"
    Write-Host "Then run: darwin"
    Write-Host "If that path is an old package-manager install, remove it or put $installBinDir first on PATH."
  }

  Write-Host "Darwin $resolvedVersion installed successfully."
} finally {
  if (Test-Path $tmpDir) {
