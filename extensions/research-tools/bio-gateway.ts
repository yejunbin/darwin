...
import { extractPaperSections } from "./bio-sections.js";
...
export function registerBioTools(pi: ExtensionAPI): void {
	pi.registerTool({
		name: "alpha_search",
		label: "Alpha Search",
		description:
			"Search research papers through bioRxiv. Modes: semantic (default, use 2-3 sentence queries), keyword (exact terms), agentic (broad multi-turn retrieval), both, or all.",
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			paper: Type.String({ description: "arXiv ID, arXiv URL, or bioRxiv URL." }),
